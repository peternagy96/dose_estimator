...
class Tester(object):
    def __init__(self, data, model, date_time, out_path):
        self.data = data
        self.model = model
 ...
 # Return a generated slice from all train and test images 
    ...
    def test_jpg(self, epoch: int, mode: str = 'forward', index: int = 40, pat_num: list = [32, 5], mods: list = ['CT', 'PET', 'SPECT']):
        ...
        if mode == 'forward':
            num_train_samples = self.model.A_train.shape[0]
            num_test_samples = self.data.A_test.shape[0]
            # process training images
            for idx in np.arange(index, num_train_samples,pat_num[0]):
                pred = self.model.G_A2B.predict(self.data.A_train[np.newaxis,idx,:,:]).squeeze()
            ...
            for idx in np.arange(index, num_test_samples, pat_num[1]):
                pred = self.model.G_A2B.predict(self.data.A_test[np.newaxis,idx,:,:]).squeeze()
            ...
            for idx in np.arange(index, num_train_samples, pat_num):
                pred = self.model.G_B2A.predict(self.data.B_train[np.newaxis,idx,:,:]).squeeze()
                self.save_basic_plot(self.data.B_train[idx], pred, self.data.A_train[idx], f"{path_name}/train_{idx}.png", [mods[-1], mods[-1], f"{mods[0]/mods[1]}"])
            # process test images
            for idx in np.arange(index, num_test_samples, pat_num):
                pred = self.model.G_B2A.predict(self.data.B_test[np.newaxis,idx,:,:]).squeeze()
                self.save_basic_plot(self.data.B_test[idx], pred, self.data.A_test[idx], f"{path_name}/test_{idx}.png", [mods[-1], mods[-1], f"{mods[0]/mods[1]}"])
     ...
     # load txt file of test file names
        ...
        for idx, i in enumerate(indices):
            ...
            for j in range(nifti_in_A.shape[0]):
                pred_B[j] = self.model.G_A2B.predict(np.stack((in1[j], in2[j]), axis=2)[np.newaxis,:,:,:]).squeeze()[:,:,0]#.reshape((256,128))
...
# Test and return 3D NIFTI images ==============================================
    ...
    def predict_nifti(self, image, direction):
        ...
        if direction == "A2B":
            for i in range(array.shape[0]):
                pred[i] = self.model.G_A2B.predict(array[np.newaxis, i,:,:,np.newaxis]).squeeze()
                #pred[i] = (255.0 / (pred[i].max() - pred[i].min()) * (pred[i] - pred[i].min())).astype(np.uint8)
                pred[i] = self.hist_match(pred[0], pred[i])
        else:
            for i in range(array.shape[0]):
                pred[i] = self.model.G_B2A.predict(array[np.newaxis, i,:,:,np.newaxis]).squeeze()
                #pred[i] = (255.0 / (pred[i].max() - pred[i].min()) * (pred[i] - pred[i].min())).astype(np.uint8)
                pred[i] = self.hist_match(pred[0], pred[i])
