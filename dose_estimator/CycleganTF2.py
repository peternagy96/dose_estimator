...
# Help functions
    ...
    def lse(self, y_true, y_pred):
        loss = tf.reduce_mean(input_tensor=tf.math.squared_difference(y_pred, y_true))
        return loss

    def cycle_loss(self, y_true, y_pred):
        loss = tf.reduce_mean(input_tensor=tf.abs(y_pred - y_true))
        return loss
...
class ReflectionPadding2D(Layer):
    ...
    def call(self, x, mask=None):
        ...
        return tf.pad(tensor=x, paddings=[[0, 0], [h_pad, h_pad], [w_pad, w_pad], [0, 0]], mode='REFLECT')
